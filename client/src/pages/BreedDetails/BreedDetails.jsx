      ...
      <div className='max-w-4xl mx-auto px-5'>
        ...
        <div className='space-y-5'>
          ...
          <div className='flex flex-col pb-20'>
            ...
            {/* Facts Section */}
            <div className='mt-5'>
              <h1 className='text-4xl font-black text-blue-950'>Facts & Health Tips</h1>

              {breed.facts && breed.facts.length > 0 ? (
                <div className='mt-3'>
                  <h3 className='text-lg font-semibold text-blue-950'>Quick Facts</h3>
                  <ul className='mt-2 list-disc list-inside text-gray-700 space-y-2'>
                    {breed.facts.map((fact, idx) => (
                      <li key={idx} className='italic'>{fact}</li>
                    ))}
                  </ul>
                </div>
              ) : (
                <p className='text-gray-600 mt-3'>No facts available.</p>
              )}

              {breed.healthTips && breed.healthTips.length > 0 && (
                <div className='mt-6'>
                  <h3 className='text-lg font-semibold text-blue-950'>Health Tips</h3>
                  <ul className='mt-2 list-disc list-inside text-gray-700 space-y-1'>
                    {breed.healthTips.map((tip, idx) => (
                      <li key={idx}>{tip}</li>
                    ))}
                  </ul>
                </div>
